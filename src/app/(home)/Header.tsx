...
export default function Header({ pageLinks, fragLinks }: { pageLinks: HeaderLink[]; fragLinks?: HeaderLink[] }) {
  ...
  function makeHeaderLink({ id, name, href, type }: HeaderLink): JSX.Element {
    switch (type) {
      case "fragment":
        return <a href={href} className="glow-hover italic">{name}</a>;
      case "link":
        return <Link href={href} className={`glow-hover${inPath(href) ? " before:content-['>_']" : ""}`}>{name}</Link>;
      case "external":
        return <a href={href} className="glow-hover group" target="_blank">{name}&nbsp;<FontAwesomeIcon icon={faArrowUpRightFromSquare} className="group-hover:[transform:scale(1.25)] transition-transform" /></a>;
    }
  }
  ...
  return (
    <header className={`space-x-10 lg:space-x-0 lg:sticky lg:top-0 flex lg:flex-col max-h-screen p-6 lg:p-24 bg-gradient-to-b lg:bg-gradient-to-r from-slate-900/60 from-30% flex-shrink-0 ${HEADING_FONT.className}`}>
      ...
      <ul className={`text-xl flex-grow` + (SHOW_FRAG_LINKS ? ' lg:flex-grow-0' : '')}>
